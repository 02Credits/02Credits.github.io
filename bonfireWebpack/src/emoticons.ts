...
export var ourEmoticons = {
  ...
  "LizardMad": "https://i.imgur.com/eXU9sSv.png"
}

export var textEmoticons: { [id: string]: string } = {
  "Denko": "(´・ω・｀)",
  "Lenny": "( ͡° ͜ʖ ͡°)"
}

export var emoticons: { [id: string]: string } = {};
Object.assign(emoticons, twitchEmoticons);
Object.assign(emoticons, ourEmoticons);

var purgeRegex = /Purge/g;
var purgeEmoticon = "<img src=\"http://imgur.com/VbXDnsr.png\" border=\"0\" text-align=\"center !important\" height=\"2000px\" alt=\"Purge\" title=\"Purge emoticon\" style=\"image-rendering: pixelated;\"/>";

export function genEmoticon (key:string, big: boolean) {
  if (big) {
...
export function replace(text: string, id: string, author: string, big: boolean) {
  if (purgeRegex.test(text) && big) {
    text = text.replace(purgeRegex, purgeEmoticon);
  } else {
    for (var key in textEmoticons) {
      var value = textEmoticons[key];
...
export function singleEmoticon(text: string) {
