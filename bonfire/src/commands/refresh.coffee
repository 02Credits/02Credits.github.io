define ["arbiter"], (arbiter) ->
  ...
  if (!localStorage.refresh#{id}) {
    ...
    document.setInterval(function () {
      ...
      var smokeKeyFrames = "@keyframes Smoke" + smokeId + " { opacity: 0; bottom: -200px; transform: rotate("+ Math.random() * 360 + "deg); } 80% { opacity: 0.8; } 100% { opacity: 0; transform: rotate(+ "Math.random() * 360 + "deg); bottom: 50px; }"
